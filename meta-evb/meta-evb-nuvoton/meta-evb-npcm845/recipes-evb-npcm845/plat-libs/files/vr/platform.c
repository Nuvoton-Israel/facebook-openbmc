...
#include <facebook/evb-npcm845_common.h>
#include "raa_gen3.h"
...
static int
_evb_npcm845_vr_rdwr(uint8_t bus, uint8_t addr, uint8_t *txbuf, uint8_t txlen,
...
static int
evb_npcm845_vr_rdwr(uint8_t bus, uint8_t addr, uint8_t *txbuf, uint8_t txlen,
              uint8_t *rxbuf, uint8_t rxlen) {
  return _evb_npcm845_vr_rdwr(bus, addr, txbuf, txlen, rxbuf, rxlen, NONE_INTF);
};

static int
rbf_vr_rdwr(uint8_t bus, uint8_t addr, uint8_t *txbuf, uint8_t txlen,
              uint8_t *rxbuf, uint8_t rxlen) {
  return _evb_npcm845_vr_rdwr(bus, addr, txbuf, txlen, rxbuf, rxlen, FEXP_BIC_INTF);
};
...
struct vr_info evb_npcm845_vr_list[] = {
  {
    .bus = SB_VR_BUS,
    .addr = VCCIN_ADDR,
...
void evb_npcm845_vr_device_check(void){
  uint8_t rbuf[16], rlen;
  int i;
  for (i = VR_CL_VCCIN; i <= VR_CL_VCCINFAON; i++) {
    rlen = sizeof(rbuf);
    if (bic_get_vr_device_id(slot_id, rbuf, &rlen, evb_npcm845_vr_list[i].bus,
                             evb_npcm845_vr_list[i].addr, NONE_INTF) < 0) {
      continue;
    }

    switch (rlen) {
      case 2:
        evb_npcm845_vr_list[i].ops = &ifx_ops;
        break;
      case 6:
        evb_npcm845_vr_list[i].ops = &ti_ops;
        break;
      default:
        evb_npcm845_vr_list[i].ops = &rns_ops;
        break;
    }
  }
}
...
void halfdome_vr_device_check(void){
  ...
  for (int i = VR_HD_VDDCR_CPU0; i <= VR_HD_VDD11S3; i++) {
    evb_npcm845_vr_list[i].addr = hd_vr_addr_table[board_rev][i - VR_HD_VDDCR_CPU0] ;
    evb_npcm845_vr_list[i].ops = hd_vr_ops_list[board_rev];
  }
}
...
void rbf_vr_device_check(void){
  ...
  for (i = VR_RBF_A0V8; i <= VR_RBF_VDDQCD; i++) {
    rlen = sizeof(rbuf);
    if (bic_get_vr_device_id(slot_id, rbuf, &rlen, evb_npcm845_vr_list[i].bus,
                             evb_npcm845_vr_list[i].addr, FEXP_BIC_INTF) < 0) {
      continue;
    }

    switch (rlen) {
      case 2:
        evb_npcm845_vr_list[i].ops = &ifx_xdpe12284_ops;
        break;
      default:
        evb_npcm845_vr_list[i].ops = &rns_ops;
        break;
...
void greatlakes_vr_device_check(void){
  evb_npcm845_vr_list[VR_CL_VCCIN].addr = GL_VCCIN_ADDR;
  evb_npcm845_vr_list[VR_CL_VCCD].addr = GL_VCCD_ADDR;
  evb_npcm845_vr_list[VR_CL_VCCINFAON].addr = GL_VCCINFAON_ADDR;
}

int plat_vr_init(void) {
  int config_status = 0;
  int vr_cnt = sizeof(evb_npcm845_vr_list)/sizeof(evb_npcm845_vr_list[0]);
  uint8_t type_1ou = TYPE_1OU_UNKNOWN;
  int server_type = SERVER_TYPE_NONE;

  server_type = evb_npcm845_common_get_slot_type(slot_id);
  if (server_type < 0) {
  ...
  for (int i = 0; i < vr_cnt; i++) {
    evb_npcm845_vr_list[i].slot_id = slot_id;
  }

  return vr_device_register(evb_npcm845_vr_list, vr_cnt);
}
