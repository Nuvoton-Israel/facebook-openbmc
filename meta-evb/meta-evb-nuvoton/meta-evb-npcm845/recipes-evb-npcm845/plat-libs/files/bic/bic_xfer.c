...
int is_bic_ready(uint8_t slot_id, uint8_t intf __attribute__((unused))) {
  ...
  ret = evb_npcm845_common_is_bic_ready(slot_id, &bic_ready);
  if (!ret && bic_ready != 1) {
...
int bic_ipmb_wrapper(uint8_t slot_id, uint8_t netfn, uint8_t cmd,
  ...
  ret = evb_npcm845_common_get_bus_id(slot_id);
  if (ret < 0) {
  ...
  while (retry < IPMB_RETRY_TIME) {
    ...
    ret = evb_npcm845_common_server_stby_pwr_sts(slot_id, &status_12v);
