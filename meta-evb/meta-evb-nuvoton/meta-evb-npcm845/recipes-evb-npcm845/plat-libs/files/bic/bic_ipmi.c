    ...
    case FW_2OU_CPLD:
      {
        ...
        if ( evb_npcm845_common_get_2ou_board_type(slot_id, &board_type) < 0 ) {
...
bic_get_dev_info(uint8_t slot_id, uint8_t dev_id, uint8_t *nvme_ready, uint8_t *status, uint8_t *type) {
  ...
  ret = evb_npcm845_common_get_2ou_board_type(slot_id, &type_2ou);
  if ( ret < 0 ) {
...
bic_get_dev_power_status(uint8_t slot_id, uint8_t dev_id, uint8_t *nvme_ready, uint8_t *status, \
  ...
  if (intf == FEXP_BIC_INTF) {
    ...
    table = 0;
    ret = evb_npcm845_common_get_2ou_board_type(slot_id, &board_type);
    if (ret < 0) {
...
bic_set_dev_power_status(uint8_t slot_id, uint8_t dev_id, uint8_t status, uint8_t intf) {
  ...
  do {
    ...
    bus_num = evb_npcm845_common_get_bus_id(slot_id) + 4;
...
bic_get_mb_index(uint8_t *index) {
  ...
  if (rlen == sizeof(GET_MB_INDEX_RESP)) {
    *index = resp.index;
    ...
    syslog(LOG_WARNING, "%s(): wrong response length (%d), while getting MB index, expected = %zd",
...
bic_get_tps_remaining_wr(uint8_t fru_id, uint8_t addr, uint16_t *remain) {
  ...
  *remain = ((rbuf[0] << 8) | rbuf[1]); //higher byte first
  ...
  if (evb_npcm845_is_zero_checksum_valid(rbuf, TI_VR_REMAIN_WR_SIZE) == false) {
...
bic_set_tps_remaining_wr(uint8_t fru_id, uint8_t addr, uint16_t remain) {
  ...
  tbuf[2] = 0; //write
  tbuf[3] = VR_REMAINING_WRITE_START_ADDR; //offset
  tbuf[4] = (evb_npcm845_common_get_slot_type(fru_id) == SERVER_TYPE_HD)
  ...
  tbuf[7] = evb_npcm845_zero_checksum_calculate(&tbuf[5], TI_VR_REMAIN_WR_SIZE); //Zero checksum
  tlen = 8;
