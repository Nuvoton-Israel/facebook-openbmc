...
pal_set_VF_power_off(uint8_t fru) {
  ...
  bus = evb_npcm845_common_get_bus_id(fru) + 4;
...
server_power_12v_on(uint8_t fru) {
  ...
  sleep(1);
  ret = evb_npcm845_common_set_fru_i2c_isolated(fru, GPIO_VALUE_HIGH);
  if ( ret < 0 ) {
...
server_power_12v_off(uint8_t fru) {
  ...
  ret = evb_npcm845_common_set_fru_i2c_isolated(fru, GPIO_VALUE_LOW);
  if (ret < 0) {
...
pal_get_server_12v_power(uint8_t slot_id, uint8_t *status) {
  ...
  ret = evb_npcm845_common_server_stby_pwr_sts(slot_id, status);
  if ( ret < 0 ) {
    syslog(LOG_WARNING, "%s: Failed to run evb_npcm845_common_server_stby_pwr_sts on slot%d\n", __func__, slot_id);
  }
...
pal_get_server_power(uint8_t fru, uint8_t *status) {
  ...
  ret = evb_npcm845_common_check_slot_id(fru);
  if ( ret < 0 ) {
...
pal_is_valid_expansion_dev(uint8_t slot_id, uint8_t dev_id, uint8_t *rsp) {
  ...
  if ( (dev_id < st_2ou_idx) && ((config_status & PRESENT_1OU) == PRESENT_1OU) ) {
    ...
    uint8_t type = 0xff;
    if ( evb_npcm845_common_get_2ou_board_type(slot_id, &type) < 0 ) {
      return POWER_STATUS_FRU_ERR;
...
pal_get_device_power(uint8_t slot_id, uint8_t dev_id, uint8_t *status, uint8_t *type) {
  ...
  if (evb_npcm845_common_check_slot_id(slot_id) == 0) {
