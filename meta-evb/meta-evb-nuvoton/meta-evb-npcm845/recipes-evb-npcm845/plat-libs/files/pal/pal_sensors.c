...
get_skip_sensor_list(uint8_t fru, uint8_t **skip_sensor_list, int *cnt, const uint8_t bmc_location, const uint8_t config_status) {
  ...
  server_type = evb_npcm845_common_get_slot_type(fru);
  if (server_type < 0) {
  ...
  if (bic_dynamic_skip_sensor_list[fru-1][0] == 0) {
    ...
    if (((config_status & PRESENT_2OU) == PRESENT_2OU) &&
        (evb_npcm845_common_get_2ou_board_type(fru, &type_2ou) == 0)) {
...
pal_get_fru_sensor_list(uint8_t fru, uint8_t **sensor_list, int *cnt) {
  ...
  case FRU_SLOT4:
    ...
    // 2OU
    if ( (config_status & PRESENT_2OU) == PRESENT_2OU ) {
      ret = evb_npcm845_common_get_2ou_board_type(fru, &board_type);
      if (ret < 0) {
...
read_pdb_cl_vdelta(uint8_t snr_number, float *value) {
  ...
  if (evb_npcm845_common_get_slot_type(FRU_SLOT1) == SERVER_TYPE_HD || evb_npcm845_common_get_slot_type(FRU_SLOT3) == SERVER_TYPE_HD) {
...
#define BIC_SENSOR_READ_NA 0x20
  ...
  if(sensor.read_type == ACCURATE_CMD_4BYTE) {
    *value = ((int16_t)((sensor.value & 0xFFFF0000) >> 16)) * 0.001 + ((int16_t)(sensor.value & 0x0000FFFF)) ;
    ...
    if (evb_npcm845_common_get_slot_type(fru) == SERVER_TYPE_HD) {
    ...
    } else if (evb_npcm845_common_get_slot_type(fru) == SERVER_TYPE_CL) {
      ...
      syslog(LOG_WARNING, "%s()  fru%x, unknown slot type 0x%x", __func__, fru, evb_npcm845_common_get_slot_type(fru));
    }
    return 0;
  ...
  *value = ((m * x) + (b * pow(10, b_exp))) * (pow(10, r_exp));
  ...
  //  Apply value correction for Crater Lake
  if (evb_npcm845_common_get_slot_type(fru) == SERVER_TYPE_CL) {
    switch (sensor_num) {
      case BIC_SENSOR_FIO_TEMP:
        apply_frontIO_correction(fru, sensor_num, value, bmc_location);
        break;
...
pal_sensor_read_raw(uint8_t fru, uint8_t sensor_num, void *value) {
  ...
  if (evb_npcm845_common_get_bb_hsc_type(&hsc_type) != 0) {
    goto skip_hsc_init;
  }
...
pal_sensor_sdr_init(uint8_t fru, sensor_info_t *sinfo) {
  ...
  while ( prsnt_retry-- > 0 ) {
    ...
    if ( (config_status & PRESENT_2OU) == PRESENT_2OU ) {
      ...
      evb_npcm845_common_get_2ou_board_type(fru, &board_type);
    }
    break;
  }
  if ( ret < 0 ) {
