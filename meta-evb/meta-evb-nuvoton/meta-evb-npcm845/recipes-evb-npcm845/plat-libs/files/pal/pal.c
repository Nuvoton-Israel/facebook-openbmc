...
pal_get_80port_record(uint8_t slot_id, uint8_t *res_data, size_t max_len, size_t *res_len) {
  ...
  ret = evb_npcm845_common_check_slot_id(slot_id);
  if (ret < 0 ) {
    ret = PAL_ENOTSUP;
    goto error_exit;
  }
...
pal_get_board_id(uint8_t slot, uint8_t *req_data, uint8_t req_len, uint8_t *res_data, uint8_t *res_len) {
  ...
  *data++ = bmc_location;
  ...
  if (bmc_location == BB_BMC) {
    ret = evb_npcm845_common_get_bb_rev();
    if (ret < 0) {
      *data++ = 0x00;
    } else {
      *data++ = ret;
  ...
  *data++ = 0x00;  // slot type. server = 0x00
  ...
  if (evb_npcm845_common_check_slot_id(slot) != 0) {
...
pal_is_fru_prsnt(uint8_t fru, uint8_t *status) {
  ...
  switch (fru) {
    ...
    case FRU_SLOT3:
    case FRU_SLOT4:
      if (evb_npcm845_common_is_fru_prsnt(fru, status) < 0) {
        return -1;
      }
      break;
    case FRU_BB:
...
pal_get_fru_id(char *str, uint8_t *fru) {
  ...
  ret = evb_npcm845_common_get_fru_id(str, fru);
  if ( ret < 0 ) {
...
pal_get_fruid_name(uint8_t fru, char *name) {
  return evb_npcm845_get_fruid_name(fru, name);
}
...
pal_dev_fruid_write(uint8_t fru, uint8_t dev_id, char *path) {
  ...
  if ( (dev_id == BOARD_1OU) && ((config_status & PRESENT_1OU) == PRESENT_1OU) && (bmc_location != NIC_BMC) ) { // 1U
    ...
    if ( evb_npcm845_common_get_2ou_board_type(fru, &type_2ou) < 0 ) {
...
pal_get_sysfw_ver_from_bic(uint8_t slot_id, uint8_t *ver) {
  ...
  if (evb_npcm845_common_get_slot_type(slot_id) == SERVER_TYPE_HD) {
...
int pal_get_poss_pcie_config(uint8_t slot, uint8_t *req_data, uint8_t req_len, uint8_t *res_data, uint8_t *res_len) {
  ...
  if ( (config_status & PRESENT_2OU) == PRESENT_2OU ) {
    if ( evb_npcm845_common_get_2ou_board_type(slot, &type_2ou) < 0 ) {
...
{
  int ret = evb_npcm845_common_get_slot_type(fru);
    ...
    case FRU_SLOT4:
      switch(sensor_num) {
        ...
        case BIC_SENSOR_SSD_HOT_PLUG:
          if (evb_npcm845_common_get_2ou_board_type(fru, &board_type) < 0) {
...
pal_sel_root_port_mapping_tbl(uint8_t fru, uint8_t *bmc_location, MAPTOSTRING **tbl, uint8_t *cnt) {
  ...
  do {
    ...
    if ( ((config_status & PRESENT_2OU) == PRESENT_2OU) ) {
      ret = evb_npcm845_common_get_2ou_board_type(fru, &board_2u);
      if (ret < 0) {
...
pal_parse_pmic_err_event(uint8_t fru, uint8_t *event_data, char *error_log) {
  ...
  if (evb_npcm845_common_get_slot_type(fru) == SERVER_TYPE_HD) {
...
pal_store_crashdump(uint8_t fru, bool ierr) {
  ...
  return evb_npcm845_common_crashdump(fru, ierr, false);
}
  ...
  switch (snr_num) {
    ...
    case BIC_SENSOR_SYSTEM_STATUS:
      ...
      if (event_data[3] == 0x11) { // when another blade insert/remove, start/stop fscd
        ...
        evb_npcm845_common_fscd_ctrl(event_data[DATA_INDEX_2]);
...
pal_get_dev_id(char *str, uint8_t *dev) {
  return evb_npcm845_common_dev_id(str, dev);
}
...
pal_get_dev_fruid_name(uint8_t fru, uint8_t dev, char *name)
{
  ...
  switch(fru) {
    ...
    case FRU_SLOT3:
    case FRU_SLOT4:
      evb_npcm845_common_dev_name(dev, dev_name);
      break;
    default:
      return -1;
  }
...
pal_get_dev_name(uint8_t fru, uint8_t dev, char *name)
{
  switch(fru) {
    ...
    case FRU_SLOT3:
    case FRU_SLOT4:
      evb_npcm845_common_dev_name(dev, name);
      break;
    default:
      return -1;
  }
...
pal_get_dev_fruid_path(uint8_t fru, uint8_t dev_id, char *path) {
  return evb_npcm845_get_fruid_path(fru, dev_id, path);
}
...
pal_get_cpld_ver(uint8_t fru, uint8_t *ver) {
  ...
  switch (fru) {
    ...
    case FRU_SLOT4:
      ...
      i2c_bus = evb_npcm845_common_get_bus_id(fru) + 4;
...
pal_check_slot_fru(uint8_t slot_id) {
  int slot_type = evb_npcm845_common_get_slot_type(slot_id);
...
{
  return evb_npcm845_common_is_prot_card_prsnt(fru);
}
...
pal_read_bic_sensor(uint8_t fru, uint8_t sensor_num, ipmi_extend_sensor_reading_t *sensor, uint8_t bmc_location, const uint8_t config_status) {
  ...
  if (((config_status & PRESENT_2OU) == PRESENT_2OU) && (board_type[fru-1] == UNKNOWN_BOARD)) {
    ret = evb_npcm845_common_get_2ou_board_type(fru, &board_type[fru-1]);
    if (ret < 0) {
      syslog(LOG_ERR, "%s() Cannot get board_type", __func__);
    }
  }
