...
int
evb_npcm845_common_set_fru_i2c_isolated(uint8_t fru, uint8_t val) {
  return gpio_set_value_by_shadow(gpio_server_i2c_isolated[fru], (val==0)?GPIO_VALUE_LOW:GPIO_VALUE_HIGH);
}

int
evb_npcm845_common_get_fru_id(char *str, uint8_t *fru) {
  int fru_id = 0;
  bool found_id = false;
...
int
evb_npcm845_common_get_slot_id(char *str, uint8_t *fru) {
  int fru_id = 0;
  bool found_id = false;
...
int
evb_npcm845_common_check_slot_id(uint8_t fru) {
...
int
evb_npcm845_common_is_fru_prsnt(uint8_t fru, uint8_t *val) {
  uint8_t bmc_location = 0;
...
int
evb_npcm845_common_server_stby_pwr_sts(uint8_t fru, uint8_t *val) {
  int ret = 0;
  uint8_t bmc_location = 0;
  gpio_value_t gpio_value;
...
static int
evb_npcm845_read_sb_cpld(uint8_t fru, uint8_t offset, uint8_t *data) {
  int ret, i2cfd;
  uint8_t bus = 0;
  uint8_t tbuf[8] = {0};
  uint8_t rbuf[8] = {0};

  ret = evb_npcm845_common_get_bus_id(fru);
  if (ret < 0) {
...
static int
evb_npcm845_read_sb_cpld_checked(uint8_t fru, uint8_t offset, uint8_t *data) {
  ...
  if (evb_npcm845_common_check_slot_id(fru)) {
    syslog(LOG_WARNING, "%s: Unknown fru: %u", __func__, fru);
    return -1;
  }

  if (evb_npcm845_common_is_fru_prsnt(fru, &prsnt)) {
    return -1;
  }
  if (!prsnt) {
    return -1;
  ...
  if (evb_npcm845_common_server_stby_pwr_sts(fru, &stby_pwr)) {
    return -1;
  }
  if (!stby_pwr) {
    return -1;
  ...
  if (retry_cond(!evb_npcm845_read_sb_cpld(fru, offset, data), 3, 50)) {
    return -1;
  }
...
int
evb_npcm845_common_is_bic_ready(uint8_t fru, uint8_t *val) {
  uint8_t rbuf[8] = {0};

  if (evb_npcm845_read_sb_cpld(fru, CPLD_REG_BIC_READY, rbuf)) {
    return -1;
  }
  *val = (rbuf[0] & 0x2) >> 1;
...
int
evb_npcm845_common_get_bus_id(uint8_t slot_id) {
  int bus_id = 0;
  switch(slot_id) {
    case FRU_SLOT1:
...
int
evb_npcm845_common_get_bmc_location(uint8_t *id) {
...
int
evb_npcm845_common_get_slot_type(uint8_t fru) {
  ...
  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_BOARD, (uint8_t *)value)) {
    return -1;
  }
...
int
evb_npcm845_common_get_sb_rev(uint8_t fru) {
  int ret = 0;
  char key[MAX_KEY_LEN];
  char value[MAX_VALUE_LEN] = {0};
  ...
  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_REV_ID, (uint8_t *)value)) {
    return -1;
  }
...
int
evb_npcm845_common_get_1ou_m2_prsnt(uint8_t fru) {
  int ret = 0;
  char key[MAX_KEY_LEN];
  char value[MAX_VALUE_LEN] = {0};

  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_M2_PRSNT, (uint8_t *)value)) {
    return -1;
  }
...
int
evb_npcm845_common_get_sb_pch_bic_pwr_fault(uint8_t fru) {
  int ret = 0;

  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_PCH_BIC_PWR_FAULT, (uint8_t *)&ret)) {
    return -1;
  }
...
int
evb_npcm845_common_get_sb_cpu_pwr_fault(uint8_t fru) {
  int ret = 0;

  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_CPU_PWR_FAULT, (uint8_t *)&ret)) {
    return -1;
  }
...
int
evb_npcm845_common_get_sb_bic_boot_strap(uint8_t fru) {
  int ret = 0;

  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_SB_BIC_BOOT_STRAP, (uint8_t *)&ret)) {
    return -1;
  }
...
static int
evb_npcm845_read_bb_cpld(uint8_t bus, uint8_t offset, uint8_t *data) {
  int ret, i2cfd;
  uint8_t tbuf[8] = {0};
  uint8_t rbuf[8] = {0};
...
int
evb_npcm845_common_get_bb_rev(void) {
  int ret = 0;
  uint8_t bmc_location = 0;
  ...
  if (retry_cond(!evb_npcm845_read_bb_cpld(bus, CPLD_REG_BB_REV, (uint8_t *)value), 3, 50)) {
    return -1;
  }
...
int
evb_npcm845_common_crashdump(uint8_t fru, bool ierr __attribute__((unused)), bool platform_reset) {
  int ret = 0;
  char cmd[128] = "\0";
...
int
evb_npcm845_common_dev_id(char *str, uint8_t *dev) {
...
#ifdef DEBUG
    syslog(LOG_WARNING, "evb_npcm845_common_dev_id: Wrong fru id");
#endif
    return -1;
  }
...
int
evb_npcm845_common_dev_name(uint8_t dev, char *str) {
...
int
evb_npcm845_common_get_2ou_board_type(uint8_t fru, uint8_t *board_type) {
  char key[MAX_KEY_LEN];
  char value[MAX_VALUE_LEN] = {0};
  ...
  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_RISER, board_type)) {
    return -1;
  }
...
int
evb_npcm845_common_fscd_ctrl (uint8_t mode) {
  int ret = 0;
...
int
evb_npcm845_common_check_image_md5(const char* image_path, int cal_size, uint8_t *data, bool is_first, uint8_t comp, uint8_t board_id) {
...
int
evb_npcm845_common_check_image_signature(uint8_t board_id, uint8_t *data) {
...
int
evb_npcm845_common_get_img_ver(const char* image_path, char* ver, uint8_t comp) {
  int fd = 0;
  int byte_num = 0;
  int ret = 0;
...
uint8_t
evb_npcm845_zero_checksum_calculate(uint8_t *buf, uint8_t len) {
  uint8_t i, ret = 0;
...
bool
evb_npcm845_is_zero_checksum_valid(uint8_t *buf, uint8_t len) {
  uint8_t i, ret = 0;
...
bool
evb_npcm845_common_is_valid_img(const char* img_path, uint8_t comp, uint8_t board_id, uint8_t rev_id) {
  ...
  if (evb_npcm845_common_check_image_signature(board_id, img_info.plat_sig) < 0) {
    return false;
  }

  if (evb_npcm845_common_check_image_md5(img_path, cal_size, img_info.md5_sum, true, comp, board_id) < 0) {
    return false;
  }

  if (evb_npcm845_common_check_image_md5(img_path, info_offs, img_info.md5_sum_second, false, comp, board_id) < 0) {
    return false;
  }
...
int
evb_npcm845_common_get_bb_hsc_type(uint8_t* type) {
  char value[MAX_VALUE_LEN] = {0};
  const char *shadows[] = {
    "HSC_BB_BMC_DETECT0",
    "HSC_BB_BMC_DETECT1",
...
bool
evb_npcm845_common_is_prot_card_prsnt(uint8_t fru) {
  char key[MAX_KEY_LEN];
  char value[MAX_VALUE_LEN] = {0};
  ...
  if (evb_npcm845_read_sb_cpld_checked(fru, CPLD_REG_PROT, (uint8_t *)value)) {
    return false;
  }
