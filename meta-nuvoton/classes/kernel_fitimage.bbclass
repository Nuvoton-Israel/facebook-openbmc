...
DEPENDS:append = " u-boot-nuvoton"

IMAGE_PREPROCESS_COMMAND += " generate_data_mount_dir ; "
IMAGE_POSTPROCESS_COMMAND += " flash_image_generate ; "

addtask do_prepare_bootloaders before do_image
addtask do_merge_bootloaders before do_image after do_prepare_bootloaders
...
oe_mkimage() {
    ...
    if test -n "${KDEVICETREE}"; then
        dtbcount=1
        DFT_DTB_COUNTER=1
        for DTB in ${KDEVICETREE}; do
            if echo ${DTB} | grep -q '/dts/'; then
                bbwarn "${DTB} contains the full path to the the dts file, but only the dtb name should be used."
                DTB=`basename ${DTB} | sed 's,\.dts$,.dtb,g'`
            fi
            DTB_PATH="${DEPLOY_DIR_IMAGE}/${KDEVICETREE}"
